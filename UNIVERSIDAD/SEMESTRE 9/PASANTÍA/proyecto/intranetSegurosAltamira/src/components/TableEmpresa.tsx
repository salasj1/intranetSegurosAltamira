...
interface MyData {
  id: number;
  año: number; // Agregado el atributo "año"
  mes: number;
  tipo: string;
  
  [key: string]: number | string; // Firma de índice
}
...
function TableEmpresa() {
  ...
  const obtenerNombreMes = (numeroMes:number) => {
    return nombresMeses[numeroMes - 1]; // Restamos 1 porque los arrays en JavaScript son 0-indexados
  };
      ...
      <Table striped bordered hover responsive>
        <thead>
          <tr>
            ...
            <th onClick={() => requestSort('año')}>Año</th>
            <th onClick={() => requestSort('mes')}>Mes</th>
            <th onClick={() => requestSort('tipo')}>Tipo de Recibo</th>
            <th></th>
          </tr>
        </thead>
        <tbody>
