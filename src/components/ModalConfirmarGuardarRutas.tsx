...
const ModalConfirmarGuardarRutas: React.FC<ModalConfirmarGuardarRutasProps> = ({
  ...
  const { ida, regreso } = resumenDatos || { ida: {}, regreso: {} };

  const {
    rutas: rutasOficina = [],
    horarioTrabajoDesde,
    horarioTrabajoHasta,
    horaSalida: horaSalidaCasa,
    tiempoViaje,
    haceEscalas,
    numEscalas,
  ...
  return (
    <Modal show={show} onHide={onHide} centered size="lg">
      ...
      <Modal.Body>
        ...
        {/* Actividades Ida */}
        {actividadesSeleccionadas && actividadesSeleccionadas.length > 0 && renderActividadesTable(actividadesSeleccionadas, detallesActividades, "Actividades previas IDA")}
        {/* Actividades Regreso */}
        {actividadesSeleccionadasRegreso && actividadesSeleccionadasRegreso.length > 0 && renderActividadesTable(actividadesSeleccionadasRegreso, detallesActividadesRegreso, "Actividades previas REGRESO")}
        {/* Otros datos */}
        <div style={{ marginTop: 16 }}>
          ...
          <ul>
            <li>
              Hora salida casa:{" "}
              {horaSalidaCasa
                ? horaSalidaCasa.format
                  ? horaSalidaCasa.format("HH:mm")
                  : horaSalidaCasa.toString()
                : "N/D"}
            </li>
            <li>
              Horario trabajo:{" "}
              {(
                (horarioTrabajoDesde && horarioTrabajoDesde.format
                  ? horarioTrabajoDesde.format("HH:mm")
                  : "") +
                " - " +
                (horarioTrabajoHasta && horarioTrabajoHasta.format
                  ? horarioTrabajoHasta.format("HH:mm")
                  : "")
              ).trim()}
            </li>
