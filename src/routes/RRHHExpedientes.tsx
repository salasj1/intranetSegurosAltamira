...
import { IRutogramaPayload } from '../types/rutograma.types';
...
const RRHHExpedientes: React.FC = () => {
  ...
  const [loadingRutograma, setLoadingRutograma] = useState(false);
...
const mostrarTipoDoc = (tipo: string) => tipoDocLabel[tipo] || tipo;
  ...
  const handleApproveRutograma = () => {
    // Lógica para aprobar el rutograma
    toast.success('Rutograma aprobado.');
    setShowRutogramaModal(false);
  };
    ...
    <>
              ...
              <div className={styles.detalleContenidoRRHH}>
                ...
                {rutas.length === 0 ? (
                  ...
                  <div className={styles.cardDatosPersonales} style={{padding: '1rem'}}>
                    <p>El empleado ha registrado su rutograma.</p>
      ...
      {showRutogramaModal && (
        <ModalVerRutogramaRRHH
          ...
          tiposTransporte={[]} // Debes obtener esto de algún lado, quizás un estado global o una nueva llamada a la API
          tiposActividad={[]} // Igual que tiposTransporte
        />
      )}
    </>
  );
...
export default RRHHExpedientes;import React, { useEffect, useState, useRef } from 'react';
import { useAuth } from '../auth/AuthProvider';
import axios from 'axios';
import styles from '../css/RRHHExpedientes.module.css';
import { Mosaic } from 'react-loading-indicators';
