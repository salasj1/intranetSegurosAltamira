  ...
  return (
    <div className={styleModule.routeBox} >
          ...
          return (
            ...
            >
              <div ref={rowRef} className={styleModule.routeInputRow}>
                ...
                />
                {rutas.length > 1 && (
                  <Button variant="danger" onClick={() => handleRemove(index)} disabled={disabled}>
                    <FaRegTrashAlt />
                  </Button>
                )}
              </div>
      ...
      <div className={styleModule.routeActionsRow}>
        <Button variant="secondary" onClick={handleAdd} disabled={rutas.length >= maxRutas || disabled}>
          {addLabel}
        </Button>
        <Button variant="danger" onClick={handleClear} style={{ marginLeft: '10px' }} disabled={disabled}>
          {clearLabel}
        </Button>
      </div>
    </div>
