...
const RRHHExpedientes: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Cargar tipos de actividades extras para el modal de rutograma
    axios.get(`${apiUrl}/expediente/getTipoActividadesExtra`)
      .then(res => {
        if (res.data.success) {
          // Mapeamos para garantizar que coincida con la interfaz TipoActividad { id, nombre }
          const actividades = (res.data.tiposActividadesExtra || []).map((a: any) => ({
            id: a.id ?? a.id_tipo_actividad ?? a.IdTipo,
            nombre: a.nombre ?? a.descripcion ?? a.Nombre,
          }));
          setTiposActividad(actividades);
        }
      })
  ...
  // Parsea fechas ISO (ej: "2026-04-08T00:00:00.000Z") como fecha LOCAL
  // evitando el desfase UTC que restaría un día en zonas UTC-N (como Caracas UTC-4).
  const parseDateLocal = (fechaISO?: string | Date | null): string => {
    if (!fechaISO) return '';
    const str = typeof fechaISO === 'string' ? fechaISO : fechaISO.toISOString();
                            ...
                            {empleadosRutogramaFiltrados.length === 0 ? (
                              ...
                              <tr key={emp.id} className={styles.trSolicitudesRRHH} onClick={() => handleSeleccionarDesdeTabla(emp.cod_emp)}>
                                ...
                                <td className={styles.tdRRHH}>
                                  ...
                                  {
                                    emp.status === 'Aprobado' && emp.fecha_aprobacion ? parseDateLocal(emp.fecha_aprobacion) :
                                      emp.status === 'Devuelto' && emp.fecha_rechazo ? parseDateLocal(emp.fecha_rechazo) :
                                        parseDateLocal(emp.fecha)
                                  }
                                </td>
                                <td className={styles.tdRRHH}>
                  ...
                  <div className={styles.cardDatosPersonales} style={{ padding: '1rem' }}>
                    ...
                    {(rutogramaEstado === 'Aprobado' || rutogramaEstado === 'Devuelto') && rutogramaData?.global?.nombre_completo_revisor && (
                      <div style={{ fontSize: '0.85rem', color: '#555', marginTop: '8px', borderLeft: '3px solid #003391', paddingLeft: '10px' }}>
                        ...
                        {rutogramaData.global.fecha_aprobacion && ` el ${parseDateLocal(String(rutogramaData.global.fecha_aprobacion))}`}
                        {rutogramaData.global.fechaEnvio && ` el ${parseDateLocal(String(rutogramaData.global.fechaEnvio))}`}
                      </div>
                    )}
